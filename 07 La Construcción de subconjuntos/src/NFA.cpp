...
// Establece un NFA
void NFA::set_NFA (void) 
{
  ReadFile();
}

// Funcion que ayuda a realizar las trancisiones de un estado a su proximo
unsigned int NFA::NextTransition (state aux, char simbol) const 
{
...
state NFA::FindState (unsigned int iden) const 
{
  set <state>::iterator it = states_.begin();
  for (; it !=  states_.end(); it++)
...
void NFA::ReadFile (void) 
{
  ...
  else
  {
    ...
    file_in >> states_num; //Almacena la cantidad de estados del NFA en el atributo privado
    sentinel = "";
