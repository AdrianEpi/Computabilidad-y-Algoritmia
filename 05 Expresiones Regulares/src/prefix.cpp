...
void prefix::Update(string a)
{
  ...
  for (int i = 0; i < CharacterCounter(a); i++)
  {
    temporal[i] = aux.top();
    aux.pop();
  }
...
int prefix::get_Priority(char aux)
{
  if (aux == '*')
    return 1;
  else if (aux == '&')
...
int prefix::CharacterCounter(string cadena)
{
  char tmp[cadena.size()];
  int aux = 0;
  strcpy(tmp, cadena.c_str());
